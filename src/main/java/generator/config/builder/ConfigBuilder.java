...
public class ConfigBuilder {
    ...
    private void handlerPackage(TemplateConfig template, String outputDir, PackageConfig config) {
        ...
        packageInfo.put("parent", ConstVal.PARENT);
        ...
        packageInfo.put(ConstVal.JS, joinPackage(config.getParent(), config.getJs()));
        packageInfo.put(ConstVal.VUE, joinPackage(config.getParent(), config.getVue()));

        // 生成路径信息
        pathInfo = new HashMap<>();
        pathInfo.put("parent", ConstVal.PARENT);
        ...
        if (StringUtils.isNotEmpty(template.getJs())) {
            pathInfo.put(ConstVal.JS_PATH, joinPath(outputDir, packageInfo.get(ConstVal.JS)));
        }
        if (StringUtils.isNotEmpty(template.getVue())) {
            pathInfo.put(ConstVal.VUE_PATH, joinPath(outputDir, packageInfo.get(ConstVal.VUE)));
        }
    }
    ...
    private void processTypes(StrategyConfig config) {
        ...
        if (StringUtils.isEmpty(config.getSuperControllerClass())) {
            superControllerClass = ConstVal.SUPERD_CONTROLLER_CLASS;
        } else {
    ...
    private List<TableInfo> processTable(List<TableInfo> tableList, NamingStrategy strategy, String[] tablePrefix) {
        for (TableInfo tableInfo : tableList) {
            ...
            tableInfo.setJsName(strategyConfig, NamingStrategy.capitalFirst(processName(tableInfo.getName(), strategy, tablePrefix)));
            tableInfo.setVueName(strategyConfig, NamingStrategy.capitalFirst(processName(tableInfo.getName(), strategy, tablePrefix)));
